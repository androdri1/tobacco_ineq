...
* Date: 2021.07.09
* Goal: En esta version estamos tomando los datos del genetic matching que se hizo en R
*       para cada uno de los años/quintil.
...
////////////////////////////////////////////////////////////////////////////////
// This uses the matched dataset. R saves the resulting dataset, not weights
// or similar
////////////////////////////////////////////////////////////////////////////////
...
if 1==0 {
	...
	*keep total_indiv zona edad female kids_adults ed_1 ed_2 ed_3 year quint
	rename quint quin
	gen muestraOrig=1
	gen weig=1
	tempfile orig
	...
	// Base de datos resultante del genetic matching.
	// Los pesos weig se le ponen tanto al tratamiento (fumadores de 2011) como
	// al control; pero se derivan es del control. Los "tratados" los vamos a
	// quitar, no agregan nada. Sólo vamos a dejarlos para el caso de 2011,
	// porque ahorra tiempo al momento de hacer la regresiones finales; pero
	// para mayor seguridad no se utilizan en la version de 2011
	use "$mainF\ECV_pesosAfterMatchingSmokers.dta", clear 						
																				
	rename indi id
	drop if Tr==1 & year!=2011 // Estos son los fumadores de 2011, así que sólo deben estar ese año
	replace smoker=1 if Tr==1 & year==2011
	drop Tr
	rename smoker smokers
	//.................
	
	merge n:1 id using "$mainF\ECVrepeat_preMatch.dta", keep(match)
	gen muestraOrig=0
	label var muestraOrig "Sample before matching"
	append using `orig'
	...
	* Versión neta de gastos de tabaco .........
	...
	save "$mainF\ECVrepeat_a8.dta", replace
...
if 1==0 {
	...
	tab quint year if muestraOrig==0, col
	tab quint year if muestraOrig==1, col
	...
	cd 
	cap texdoc close
	texdoc init "Sample_balance_match", replace force
	
	tex \def\sym#1{\ifmmode^{#1}\else\(^{#1}\)\fi}		
	tex \begin{table}[H]
	tex \centering
	tex \scriptsize		
	tex \caption{Matching Sample Balance \label{tab:sample_balance_match}}
	...
	foreach varDep in  $matchvar {
		...
		foreach yeo in $annios {
			qui {
				...
				* No matching ..................................................
				* Smokers ...................
				cap reg `varDep' yearB qi_2 qi_3 qi_4 qi_5 [aw=weig] if  ((muestraOrig==1 & smokers==1) & year==`yeo') | (muestraOrig==1 & smokers==1 & year==2011)  ,r 
				if _rc==0 {
					...
					mean `varDep'  if (muestraOrig==1 & smokers==1 & year==`yeo')
					mat A=r(table)
				...
				* Non Smokers ...................
				cap reg `varDep' yearB qi_2 qi_3 qi_4 qi_5 [aw=weig] if  ((muestraOrig==1 & smokers==0) & year==`yeo') | (muestraOrig==1 & smokers==1 & year==2011)  ,r 
				if _rc==0 {
					local difnm : di %7.3f _b[yearB]
					local difsenm : di %7.3f _se[yearB]			
					local tbefnm = _b[yearB]/_se[yearB]
					...
					mean `varDep'  if (muestraOrig==1 & smokers==0 & year==`yeo')
					mat A=r(table)
				...
				* With matching ..................................................
				* Smokers............................
				cap reg `varDep' yearB qi_2 qi_3 qi_4 qi_5 [aw=weig] if  ((muestraOrig==0 & smokers==1) & year==`yeo') | (muestraOrig==1 & smokers==1 & year==2011)  ,r 
				if (_rc==0 & `yeo'!=2011 ) {
					...
					mean `varDep'  if (muestraOrig==0 & smokers==1 & year==`yeo')
					mat A=r(table)
				...
				* Non-Smokers............................
				cap reg `varDep' yearB qi_2 qi_3 qi_4 qi_5 [aw=weig] if  ((muestraOrig==0 & smokers==0) & year==`yeo') | (muestraOrig==1 & smokers==1 & year==2011)  ,r 
				if (_rc==0) {
					local difm : di %7.3f _b[yearB]
					local difsem : di %7.3f _se[yearB]			
					local tbefm = _b[yearB]/_se[yearB]
					...
					mean `varDep'  if (muestraOrig==0 & smokers==0 & year==`yeo')
					mat A=r(table)
...
////////////////////////////////////////////////////////////////////////////////
// Resultados principales (Table 2)
////////////////////////////////////////////////////////////////////////////////
if 1==1{
...
*/	
	...
	* An example ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
	reg Tobacco c.qi_2#c.smokers 		c.qi_3#c.smokers 		c.qi_4#c.smokers  	 c.qi_5#c.smokers		 ///
				 ...
				 nonsmokers smokers $controles5 [aw=weig] if year==1997 , nocons
	...
	* ~ ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
		...
		forval ll=1(1)4 { // Which list of variables
			foreach varDep in ${list`ll'} {
				qui	reg `varDep' c.qi_2#c.smokers 		c.qi_3#c.smokers 		c.qi_4#c.smokers  	 c.qi_5#c.smokers		 ///
								 c.qi_2#c.nonsmokers 	c.qi_3#c.nonsmokers 	c.qi_4#c.nonsmokers  c.qi_5#c.nonsmokers	 ///
								 nonsmokers smokers $controles5 [aw=weig] if year==`y' , nocons	
	...
	if 1==1 {
		cap texdoc close
		texdoc init Table3_Results_notob, replace force
		tex {
		tex \def\sym#1{\ifmmode^{#1}\else\(^{#1}\)\fi}		
		tex \begin{table}[H]
		tex \centering
		tex \scriptsize		
		tex \caption{ SUR Estimates for variation on shares between smokers and non-smokers \label{tab:SUR_results}}
		...
		forval ll=1(1)4 { // Which list
			...
			foreach varDep in ${list`ll'}  { // Which dependent variable
				...
				foreach y in $annios {
					...
					qui reg `varDep' smokers nonsmoker [aw=weig] if year==`y' & qi_1==1, nocon
